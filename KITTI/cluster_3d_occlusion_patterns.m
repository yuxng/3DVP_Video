function idx = cluster_3d_occlusion_patterns(data, type, algorithm, K, pscale)
...
if strcmp(type, 'Car') == 1
    flag = height > 25 & occlusion < 3 & truncation < 0.5 & ...
        (strcmp(data.type, 'Car') | strcmp(data.type, 'Van'));
else
    flag = height > 25 & occlusion < 3 & truncation < 0.5 & strcmp(data.type, type);
end
fprintf('%d examples in clustering for %s\n', sum(flag), type);
